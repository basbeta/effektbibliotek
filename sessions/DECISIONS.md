...
BEKREFTET: 2026-08-04 — `migrate resolve --applied 20260804140000_case_file` kjørt via Coolify Terminal, Dockerfile CMD byttet til `migrate deploy` (commit a25de77), pushet og autodeployet uten crash-loop denne gangen. Appen kjører nå permanent på `prisma migrate deploy`. ISSUE-011 markert Resolved.
DECIDED BY: both

---

DATE: 2026-08-05
DECISION: Gjennomfør `npm audit fix --force`-oppgraderingen (next, nodemailer, transitivt postcss/sharp) som en egen CR (CR-028), etter research i stedet for blind kjøring
CONTEXT: ISSUE-012 (åpnet 2026-08-05, samme økt som `npm audit fix` uten `--force` løste 9/13 sårbarheter) hadde 4 gjenstående high-severity CVE-er som alle krever `--force` — en `next`-bump fra pinnet `16.2.6` til `16.3.0` (utenfor stated range) og en `nodemailer`-major-bump (`^8.0.7`→`^9.0.4`). Begge ble forrige økt bevisst utsatt "fordi en Next.js-major-oppgradering er en større endring som bør testes/verifiseres separat". Før implementering ble Next.js 16.3 og nodemailer v9 sine offisielle breaking-changes-lister slått opp via websøk (ikke gjettet fra treningsdata, gitt at "current month is August 2026" og disse versjonene er nyere enn kjent).
DECISION: Kjørte `npm audit fix --force`. Verifiserte spesifikt at Next.js 16.3 sine breaking changes (Node ≥20.9, TypeScript ≥5.1, forbudt synkron `params`/`searchParams`-tilgang) allerede var oppfylt (Node 22, TS 5.x, `params`/`searchParams` awaites allerede iht. tidligere Tech Stack Notes), og at nodemailer v9 sin eneste reelle breaking change (strengere TLS-sertifikatvalidering for HTTP(S)-kall GJORT AV nodemailer selv — attachment-URL-henting, OAuth2-token-endepunkt, proxy CONNECT — samt faktisk håndheving av `disableFileAccess`/`disableUrlAccess` for `raw`-meldinger) ikke berører `lib/email.ts`, som kun bruker `sendMail({from, to, cc, replyTo, subject, text})` uten attachments, OAuth2 eller `raw`. Re-pinnet `next` til eksakt `16.3.0` (audit fix satte `^16.3.0`) for å bevare prosjektets eksisterende eksakt-pin-konvensjon (samme mønster som `eslint-config-next`, som ble matchet til samme versjon manuelt). Bumpet `@types/nodemailer` til nyeste tilgjengelige `^8.0.1` — DefinitelyTyped har ingen v9-typer ennå, men API-et er uendret og ingen typefeil dukket opp.
ALTERNATIVES CONSIDERED: Fortsette å utsette oppgraderingen til produkteier ba om det spesifikt — avvist, risikoen var allerede vurdert lav nok til å håndtere nå gitt at research konkret utelukket de kjente breaking changes for denne kodebasen spesifikt, ikke bare generelt. Oppgradere uten forutgående research (ren `--force` og se om build feiler) — avvist, i tråd med lærdommen fra ISSUE-011/CR-024 om å verifisere konkret fremfor å gjette, selv når risikoen isolert sett er lav.
RATIONALE: En dokumentert, forskningsbasert vurdering av de spesifikke breaking changes mot denne kodebasens faktiske bruksmønster er tryggere enn å utsette på ubestemt tid ELLER å oppgradere blindt. CR-systemet (CLAUDE.md: "no CR = no change") krever en formell CR selv for en avhengighetsoppgradering av denne størrelsen, i motsetning til den mindre `npm audit fix` (uten `--force`) forrige økt som ble gjort direkte uten egen CR.
CONSEQUENCES: `npm audit` viser 0 sårbarheter. `npm run build`/`npx tsc --noEmit` grønne lokalt. IKKE pushet eller produksjonsverifisert i denne økten — det siste steget (push, autodeploy, manuell OTP/bruksgodkjenning-smoke-test) gjenstår og krever eksplisitt "push"-godkjenning fulgt av produkteiers bekreftelse, i tråd med prosjektets vanlige arbeidsflyt.
DECIDED BY: both

---

DATE: 2026-08-05
DECISION: Legg til `timeZone: "Europe/Oslo"` i alle datoformateringssteder i stedet for å sette `TZ` på container-nivå (CR-029)
CONTEXT: Etter push av CR-028 og en manuell smoke-test i produksjon rapporterte produkteier at klokkeslettet i bruksgodkjenning-bekreftelsesposten var feil (viste et tidspunkt ~2 timer tidligere enn faktisk innsendingstid). Undersøkelse viste at `Dockerfile` ikke setter `TZ`, så Node-containeren i Coolify kjører i UTC — og tre steder i kodebasen (`lib/email.ts` x2, `lib/format.ts` sin `formatDate()`) formaterte datoer med `nb-NO`-locale uten å spesifisere `timeZone`, og viste dermed UTC-klokketid med norsk formatering fremfor faktisk norsk lokaltid. Selve lagrede tidsstemplene (`submittedAt`, lagret som `new Date()`) var alltid korrekte UTC-instanter — bugen var isolert til visningslaget.
DECISION: Lagt til `timeZone: "Europe/Oslo"` som explicit option i alle tre `toLocale*`-kallene, i stedet for å sette en `TZ`-miljøvariabel i Dockerfile/Coolify.
ALTERNATIVES CONSIDERED: Sette `TZ=Europe/Oslo` som miljøvariabel i Dockerfile eller Coolify — ville løst symptomet globalt for containeren, men etterlater samme klasse med bugs latent for ethvert fremtidig sted som formaterer datoer, siden det ikke er en kodemessig garanti at riktig sone brukes (en fremtidig endring av containerens `TZ`, eller kjøring i et annet miljø uten samme variabel, ville brutt det igjen stille). Eksplisitt `timeZone`-option per formateringskall er mer robust — riktig sone er en del av selve koden, ikke en implisitt miljøforutsetning.
RATIONALE: IANA-tidssonen `Europe/Oslo` håndterer sommer-/vintertid-overgangen automatisk (i motsetning til en fast UTC+1/UTC+2-offset), og en eksplisitt kodenivå-løsning er portabel uavhengig av hvilken infrastruktur appen kjører på — samme prinsipp som prosjektets øvrige "isoler eksterne forutsetninger i koden, ikke i miljøet der det er mulig"-mønster (jf. `APP_URL`-beslutningen 2026-08-03).
CONSEQUENCES: Klokkeslett i bekreftelses-e-post, kopi-til-Bas-e-post, og datoer i lister/eksport viser nå korrekt norsk lokaltid uavhengig av containerens `TZ`-innstilling. Lærdom for fremtidig kode: enhver ny `toLocaleString`/`toLocaleDateString`/`Intl.DateTimeFormat`-bruk i dette prosjektet bør inkludere `timeZone: "Europe/Oslo"` eksplisitt, ikke stole på serverens standardsone.
DECIDED BY: both
