...
- CR-029 (Done lokalt, ikke pushet) — Feil klokkeslett i bruksgodkjenning-bekreftelsesepost og datovisninger: containeren kjører i UTC, tre `toLocale*`-kall manglet `timeZone`. Lagt til `timeZone: "Europe/Oslo"` i `lib/email.ts` (x2) og `lib/format.ts`. Build/tsc grønt. Venter på push-godkjenning
...
- CR-029 (klokkeslett-bugfix): `npx tsc --noEmit` ✓, `npm run build` ✓. IKKE pushet/produksjonsverifisert ennå
