...
**Oppdatering — fortsatt ikke løst etter env-var satt og redeployet:** Lenken var uendret. Årsak: `NEXT_PUBLIC_`-prefikset variabler blir bygget statisk inn i koden av Next.js ved `next build` — også i server-kode — så en variabel som ikke var tilgjengelig i selve build-steget (kun satt til "Runtime" i Coolify, ikke nødvendigvis "Buildtime", eller feil timing) blir permanent `undefined` i det kompilerte resultatet, uavhengig av hva som settes i miljøet etterpå. Siden denne verdien kun brukes server-side, skal den aldri ha hatt `NEXT_PUBLIC_`-prefiks. Omdøpt til `APP_URL` (uten prefiks, lest dynamisk ved runtime som normalt) i `lib/email.ts` og `send-approval-request/route.ts`. **Krever at variabelen i Coolify byttes fra `NEXT_PUBLIC_APP_URL` til `APP_URL`** (ny variabel, ikke bare omdøping av verdien — den gamle kan slettes).
